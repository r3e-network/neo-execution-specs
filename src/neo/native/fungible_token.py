...
class FungibleToken(NativeContract):
    ...
    def _register_methods(self) -> None:
        ...
        self._register_method("totalSupply", self.total_supply, cpu_fee=1 << 15, call_flags=CallFlags.READ_STATES)
        self._register_method("balanceOf", self.balance_of, cpu_fee=1 << 15, call_flags=CallFlags.READ_STATES)
        self._register_method("transfer", self.transfer, cpu_fee=1 << 17, storage_fee=50, 
    ...
    def mint(self, engine: Any, account: UInt160, amount: int, call_on_payment: bool = True) -> None:
        ...
        key = self._create_storage_key(PREFIX_ACCOUNT, account.data)
        item = engine.snapshot.get_and_change(key, lambda: StorageItem(self._create_account_state().to_bytes()))
        state = self._get_account_state(item)
    ...
    def transfer(self, engine: Any, from_account: UInt160, to_account: UInt160, 
        ...
        if amount == 0:
            ...
            if from_account == to_account:
                ...
                storage_to = engine.snapshot.get_and_change(key_to, lambda: StorageItem(self._create_account_state().to_bytes()))
    ...
    def _on_balance_changing(self, engine: Any, account: UInt160, state: AccountState, amount: int) -> None:
        """Called when balance is changing. Override for custom behavior."""
        pass
    
    def _post_transfer(self, engine: Any, from_account: Optional[UInt160], to_account: Optional[UInt160],
                       amount: int, data: Any, call_on_payment: bool) -> None:
