...
class CryptoLib(NativeContract):
    ...
    def _register_methods(self) -> None:
        ...
        # Hash functions
        self._register_method("sha256", self.sha256,
                            cpu_fee=1 << 15, call_flags=CallFlags.NONE)
        self._register_method("ripemd160", self.ripemd160,
                            cpu_fee=1 << 15, call_flags=CallFlags.NONE)
        self._register_method("murmur32", self.murmur32,
                            cpu_fee=1 << 13, call_flags=CallFlags.NONE)
        self._register_method("keccak256", self.keccak256,
                            cpu_fee=1 << 15, call_flags=CallFlags.NONE)
        
        # Signature verification
        self._register_method("verifyWithECDsa", self.verify_with_ecdsa,
                            cpu_fee=1 << 15, call_flags=CallFlags.NONE)
        self._register_method("verifyWithEd25519", self.verify_with_ed25519,
                            cpu_fee=1 << 15, call_flags=CallFlags.NONE)
        self._register_method("recoverSecp256K1", self.recover_secp256k1,
                            cpu_fee=1 << 15, call_flags=CallFlags.NONE)
        
        # BLS12-381 operations
        self._register_method("bls12381Serialize", self.bls12381_serialize,
                            cpu_fee=1 << 19, call_flags=CallFlags.NONE)
        self._register_method("bls12381Deserialize", self.bls12381_deserialize,
                            cpu_fee=1 << 19, call_flags=CallFlags.NONE)
        self._register_method("bls12381Equal", self.bls12381_equal,
                            cpu_fee=1 << 5, call_flags=CallFlags.NONE)
        self._register_method("bls12381Add", self.bls12381_add,
                            cpu_fee=1 << 19, call_flags=CallFlags.NONE)
        self._register_method("bls12381Mul", self.bls12381_mul,
                            cpu_fee=1 << 21, call_flags=CallFlags.NONE)
        self._register_method("bls12381Pairing", self.bls12381_pairing,
                            cpu_fee=1 << 23, call_flags=CallFlags.NONE)
