...
def newbuffer(engine: ExecutionEngine, instruction: Instruction) -> None:
    ...
    if length < 0:
        raise InvalidOperationException(f"Buffer length cannot be negative: {length}")
...
def memcpy(engine: ExecutionEngine, instruction: Instruction) -> None:
    ...
    if count < 0:
        raise InvalidOperationException(f"Count cannot be negative for MEMCPY: {count}")
    si = int(engine.pop().get_integer())
    if si < 0:
        raise InvalidOperationException(f"Source index cannot be negative: {si}")
    src = engine.pop().get_span()
    if si + count > len(src):
        raise InvalidOperationException("Source range out of bounds")
    di = int(engine.pop().get_integer())
    if di < 0:
        raise InvalidOperationException(f"Destination index cannot be negative: {di}")
    dst = engine.pop()
    if not isinstance(dst, Buffer):
        raise InvalidOperationException("Destination must be a Buffer")
    if di + count > len(dst):
        raise InvalidOperationException("Destination range out of bounds")
    dst.inner_buffer[di:di+count] = src[si:si+count]


def cat(engine: ExecutionEngine, instruction: Instruction) -> None:
    """Concatenate two byte strings. Returns Buffer per C# reference."""
    x2 = engine.pop().get_span()
    x1 = engine.pop().get_span()
    length = len(x1) + len(x2)
    engine.limits.assert_max_item_size(length)
    buf = Buffer(length)
    buf.inner_buffer[0:len(x1)] = x1
    buf.inner_buffer[len(x1):] = x2
    engine.push(buf)
...
def substr(engine: ExecutionEngine, instruction: Instruction) -> None:
    ...
    count = int(engine.pop().get_integer())
    if count < 0:
        raise InvalidOperationException(f"Count cannot be negative: {count}")
    index = int(engine.pop().get_integer())
    if index < 0:
        raise InvalidOperationException(f"Index cannot be negative: {index}")
    x = engine.pop().get_span()
    if index + count > len(x):
        raise InvalidOperationException("Range out of bounds")
    buf = Buffer(count)
    buf.inner_buffer[0:count] = x[index:index+count]
    engine.push(buf)
...
def left(engine: ExecutionEngine, instruction: Instruction) -> None:
    ...
    x = engine.pop().get_span()
    if count > len(x):
        raise InvalidOperationException(f"Count out of range: {count}")
    buf = Buffer(count)
    buf.inner_buffer[0:count] = x[:count]
    engine.push(buf)
